...
import { Search, SlidersHorizontal, HelpCircle, Megaphone, MessageCircle, ChevronDown, Check, User, MessageSquare, Reply } from "lucide-react";
...
type ParticipationFilter = "all" | "my_posts" | "replies_to_me" | "commented";
...
export function ThreadList({ activeTab, activeCategory, selectedThread, onSelectThread, activeCourse, width }: ThreadListProps) {
  ...
  const threads = allThreads.filter((t) => {
    ...
    if (filter === "my_posts") return t.author === CURRENT_USER;
    if (filter === "replies_to_me") return !!t.repliedToMe;
    if (filter === "commented") return !!t.iCommented || t.author === CURRENT_USER;
    return true;
  });
        ...
        <div className="flex justify-between items-center mt-2 relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute left-0 top-full mt-1 w-60 bg-white rounded-md shadow-lg border border-gray-200 z-20 py-1">
              ...
              {(["all", "my_posts", "replies_to_me", "commented"] as ParticipationFilter[]).map((f) => (
                <button
                  key={f}
                  onClick={() => { setFilter(f); setMenuOpen(false); }}
                  className="w-full flex items-center gap-2 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 text-left"
                >
                  <span className="text-gray-500">{filterIcon(f)}</span>
                  <span className="flex-1">{filterLabels[f]}</span>
                  {filter === f && <Check size={13} className="text-[#4a2e8a]" />}
                </button>
              ))}
            </div>
          )}
            ...
            <div className="grid grid-cols-[8px_18px_minmax(0,1fr)_auto] items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5 min-w-0">
                  ...
                  {thread.iCommented && thread.author !== CURRENT_USER && !thread.repliedToMe && (
              ...
              <div className="flex items-center gap-2 shrink-0 text-[11px] text-gray-400">
                ...
                {thread.likes && <span>♥ {thread.likes}</span>}
              </div>
            </div>
          </div>
        ))}
